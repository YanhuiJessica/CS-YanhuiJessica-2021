...
function koalainit(token, position) {
  csrf_token = token;
  pos = position;
}
...
(function() {
  ...
  Circle.addToVis = function(vis, circles, init) {
    ...
    circle = circle.attr('randkey', function(d) {
        if (d.size === 8) {
          var posdata;
          switch(pos) {
            case 1:
    ...
    if (init) {
      ...
      circle = circle
        ...
        .attr('randkey', function(d) { return d.parent.key; })
